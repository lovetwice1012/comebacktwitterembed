...
概要と重いレポートは別の構築枠を使う。全体のSQL同時実行数は最大4、重いレポートは最大3で概要の処理余地を残す。大きな一時表を使うSQLは重い側の3枠を占有し、メモリ競合で他の集計と同時にディスクへ退避しないようにする。SELECTにはDB側のMAX_EXECUTION_TIMEを付与し、WITH句は外側のSELECTに付ける。既定のSQL上限は120秒、レポート全体の新規クエリ発行予算は600秒。`DASHBOARD_REPORT_QUERY_TIMEOUT_MS`でSQL上限を変更可能。
...
時間別のユニークキーを数える単表の集計は、必要なグループ列とキーだけを先にDISTINCTで絞り、外側のCOUNTで正確な人数・サーバー数を求める。SQL単位の一時表上限は2 GiBとし、重い集計の排他枠を使う。NULLだけのグループ、空の期間、バインド値の順序も維持する。

設定効果の詳細は最新60件の監査を先に選び、サーバー・プロバイダの指定別にインデックスで事実を結合する。まとめの効果分析は、関係するサーバー・期間の加算可能な値を時間単位へ先に集約し、重なる監査期間へ結合する。従来の各監査への重複加算は維持する。個別の読み取り専用計測には scripts/probe_admin_analytics.js を使用できる。

30日分の時間帯・曜日・日別統計は、期間区分・プロバイダ・アカウント単位へ先に集約し、加算値と正確な重複除外を分けて求める。イベント別キーの読込には本番に存在する idx_provider_hourly_unique_event_key_time を指定し、新規DBのDDLにも同じインデックスを含める。URL分析は上位の候補を先に確定し、選ばれた候補の正確な人数・サーバー数を計算する。

コールド時の30日ユニークキー集計で約86秒を実測したため、SQL単位は120秒・完全レポートは600秒の予算とした。既定値の引き上げだけでなく、各SQLの結合・重複除外・読込方法を変更している。MySQLのTempTable全体のRAM 1 GiB・mmap 1 GiBは維持する。
