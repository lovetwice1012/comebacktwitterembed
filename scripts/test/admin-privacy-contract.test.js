...
test('overview refreshes automatically while detailed analyses require an explicit request', () => {
    ...
    const adminConsoleSource = fs.readFileSync(path.join(repoRoot, 'dashboard', 'components', 'admin', 'admin-console.tsx'), 'utf8');
    const adminCatalogRouteSource = fs.readFileSync(path.join(repoRoot, 'dashboard', 'app', 'api', 'admin', 'catalog', 'route.ts'), 'utf8');
    const envSource = fs.readFileSync(path.join(repoRoot, 'dashboard', 'lib', 'env.ts'), 'utf8');
    ...
    assert.match(overviewBody, /Promise\.all\(\[loadPersistedOverviewSnapshot\(\), loadPersistedAdvancedAnalyticsSnapshot\(\)\]\)/);
    assert.match(dataSource, /refreshReportSnapshot\(entry/);
    assert.match(dataSource, /enqueueAdminAnalyticsBuild\(\(\) => buildAdminDetailedAnalytics\(entry\.filters\)\)/);
    assert.match(dataSource, /enqueueAdminAnalyticsBuild\(\(\) => buildAdminGuildAnalyticsPreview\(entry\.filters\)\)/);
    assert.match(dataSource, /enqueueAdminAnalyticsBuild\(\(\) => buildAdminProviderMarketingPreview\(entry\.filters\)\)/);
    ...
    assert.match(guildPreviewBody, /void refreshAdminGuildAnalyticsPreviewCacheEntry\(entry\)\.catch/);
    ...
    assert.match(providerPreviewBody, /void refreshAdminProviderMarketingPreviewCacheEntry\(entry\)\.catch/);
    assert.doesNotMatch(providerPreviewBody, /buildAdminProviderMarketingPreview\(filters\)/);
    assert.match(envSource, /DASHBOARD_ADMIN_ANALYTICS_PREWARM/);
    assert.match(envSource, /DASHBOARD_DB_CONNECTION_LIMIT/);
    assert.match(envSource, /connection_limit/);
