...
- `GET /v1/metrics`: exact root-request counts, F/(F+D+P+E+U+X), outcome/provider counts, exact nearest-rank per-outcome latency, distinct IDs, exclusions and coverage. A zero denominator is null. A missing terminal after ten minutes (or twice a longer worker deadline) is X; this does not claim a confirmed failure. The matching run list uses the same rule. Views/read receipts/link clicks are unsupported and never fabricated.
- `GET /v1/shards`: the latest shard snapshot. `items` reports each observed discord.js shard as online/offline/unknown with status and ping; it also reports completed/started request counts for the rolling last minute and the current Asia/Tokyo calendar day. A stale or missing heartbeat is unknown, never an inferred offline state. This endpoint is separate so a slow historical request report cannot hide current shard state.
...
Rules distinguish stopped process, stale progress, failed local HTTP, local-success/public-failure and management storage pressure. Every result carries the evidence event and rule version. Unknown heartbeat does not imply healthy or hung. Auto-restart of a verified hung process requires a current process whose PID matches recorded telemetry, stale heartbeat, repeated failed local probes, fresh independent successful DB connection diagnosis, no maintenance intent and restart-budget capacity. A guardian that remains active while its Bot child disappears uses the same unit identity, repeated workload-unverified evidence and stale/unobserved heartbeat before enqueueing one bounded restart. Normal process-exit restarts remain systemd's responsibility. Worker CLI failures are never blindly retried.
