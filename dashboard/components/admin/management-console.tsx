...
export function ManagementConsole({ initialTab = "search", standalone = false }: { initialTab?: Tab; standalone?: boolean }) {
  ...
  useEffect(() => { if (tab !== "metrics" || !health?.ok || shardData) return; let cancelled = false; api("shards").then(data => { if (!cancelled) setShardData(data); }).catch(e => { if (!cancelled) setError(text(e.message)); }); return () => { cancelled = true; }; }, [tab, health, shardData]);
