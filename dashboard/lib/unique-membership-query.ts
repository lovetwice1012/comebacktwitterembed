...
export function compactUniqueMembershipCount(sql: string) {
  ...
  if (groupAt < 0) {
    ...
    return `SELECT COUNT(members.key_hash) AS ${countAlias}
      FROM (SELECT DISTINCT key_hash FROM bot_provider_hourly_unique_keys WHERE ${remainder}) members`;
  }
  ...
  return `SELECT /*+ SET_VAR(tmp_table_size=1073741824) */ ${aliases.map(value => `members.${value}`).join(',')},
    COUNT(members.key_hash) AS ${countAlias}
    FROM (SELECT DISTINCT ${projection},key_hash FROM bot_provider_hourly_unique_keys WHERE ${where}) members
